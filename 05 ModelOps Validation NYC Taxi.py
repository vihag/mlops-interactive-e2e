...
# model name from last notebook: vg_mlops_cicd_nyc_taxi_fare_model
try:
...
model_uri = f"models:/vg_mlops_cicd_nyc_taxi_fare_model/{version}"
...
try:
  ...
  with_predictions_reordered = with_predictions.select(cols)
...
#load spark model using basic mlflow
...
if not loaded_model.metadata.signature:
  ...
  #TODO when feature store api is compatible with signaure inferencing
...
webhook_url = dbutils.secrets.get("vg-gcp-wksps-scope", "mlops_webhook")
...
#skip the run, registry helpers is not available
...
if '0' in results or 'fail' in results: 
  ...
  print("Failures in validation. Rejecting the request to promote to staging!")
  ...
  print("Successfull validation. Moving to Staging!")
