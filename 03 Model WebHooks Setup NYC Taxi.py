...
# MAGIC %md
# MAGIC ## NYC Taxi Fare Prediction: Model Ops with WebHooks & MLFlow
# MAGIC 
...
#get model name from the auto_ml baseline notebook where we registered the model

#model_name = dbutils.widgets.get("vg_ibm_telco_customer_churn_lr_model")
...
trigger_job = json.dumps({
  ...
  "status": "ACTIVE",
  "job_spec": {
    "job_id": "16011",    # This is our 05_ops_validation notebook # Must create your own job - i have created a manual triggered job (no schedule)
    "workspace_url": host,
    "access_token": token
  }
})
...
#Created my own webhook - https://hooks.slack.com/services/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

slack_webhook = dbutils.secrets.get("vg-gcp-wksps-scope", "mlops_webhook") # You have to set up your own webhook!
